...
namespace EventBus.Base.SubManagers
{
    public class InMemoryEventBusSubscriptionManager : IEventBusSubscriptionManager
    {
        ...
        public Func<string, string> eventNameGetter;//kuyrukta olanların sonundaki IntegrationEvent Trim lemek için
        ...
        {
            if (!HasSubscriptionsForEvent(eventName))//event bulunup bulunmadıgına bakılır, event in subscript olup oladıgı bilgisi geliyor
            {
                _handlers.Add(eventName, new List<SubscriptionInfo>());

            }
            //dışarıdan gelen IntegrationEventHandler ve tip gelir ve varsa hata fırlatırız
        ...
        private void RemoveHandler(string eventName,SubscriptionInfo subRemove)
        {
            if (subRemove != null)
            {
                _handlers[eventName].Remove(subRemove) ;
        ...
        //event silindiginde kullanan bir yer oldugunda haber vermek için yazılan bir metod
